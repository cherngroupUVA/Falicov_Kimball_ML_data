...
path = "/scratch/sz8ea/FK/model/new_R10/model/P2_B/"

#%%
df_train = pd.read_csv("/scratch/sz8ea/FK/model/new_R10/relax/T0.05/job2/P2_B/input1.txt",sep='\t', header=None)
df_train_y = pd.read_csv("/scratch/sz8ea/FK/model/new_R10/relax/T0.05/job2/P2_B/output1.txt",sep='\t', header=None)

for i in range(1,7):
    for j in range(1,11):
        data_name_in = "/scratch/sz8ea/FK/model/new_R10/job"+str(i)+"/P2_B/"+"input"+str(j)+".txt" 
        data_name_out = "/scratch/sz8ea/FK/model/new_R10/job"+str(i)+"/P2_B/"+"output"+str(j)+".txt"
        #print(data_name_in)
        #print(data_name_out)       
...
for i in range(2,3):
    for j in range(2,101):
        data_name_in = "/scratch/sz8ea/FK/model/new_R10/relax/T0.05/job"+str(i)+"/P2_B/"+"input"+str(j)+".txt" 
        data_name_out = "/scratch/sz8ea/FK/model/new_R10/relax/T0.05/job"+str(i)+"/P2_B/"+"output"+str(j)+".txt"
        #print(data_name_in)
        #print(data_name_out)       
        in_data = pd.read_csv(data_name_in,sep='\t', header=None)
        out_data = pd.read_csv(data_name_out,sep='\t', header=None)
...
df_train_matrix_y = df_train_matrix_y[:,1]
...
#%%
import joblib
joblib.dump(scaler_x, path+'X_scaler_1.pkl')
joblib.dump(scaler_y, path+'Y_scaler_1.pkl')
#%%
...
class Net(torch.nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.conv1 = torch.nn.Sequential( #(1,212)
            torch.nn.Conv1d(1,16,5,1,0), #(16,210)
            torch.nn.ReLU(),
            torch.nn.MaxPool1d(3,3)  #(16,70)
)
...
net = net.double()
if os.access(path+"model_1.pt",os.R_OK):
    net.load_state_dict(torch.load(path + "model_1.pt"))
lrspeed=1000
lr=1/lrspeed
...
#%%
torch.save(net.state_dict(),path + "model_1.pt")
#%%
