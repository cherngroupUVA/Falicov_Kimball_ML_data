...
path = "/scratch/sz8ea/FK/model/new_R10/model/P3/"

#%%
df_train = pd.read_csv("/scratch/sz8ea/FK/model/new_R10/relax/T0.05/job2/P3/input1.txt",sep='\t', header=None)
df_train_y = pd.read_csv("/scratch/sz8ea/FK/model/new_R10/relax/T0.05/job2/P3/output1.txt",sep='\t', header=None)

for i in range(1,7):
    for j in range(1,11):
        data_name_in = "/scratch/sz8ea/FK/model/new_R10/job"+str(i)+"/P3/"+"input"+str(j)+".txt" 
        data_name_out = "/scratch/sz8ea/FK/model/new_R10/job"+str(i)+"/P3/"+"output"+str(j)+".txt"
        #print(data_name_in)
        #print(data_name_out)       
...
for i in range(2,3):
    for j in range(2,24):
        data_name_in = "/scratch/sz8ea/FK/model/new_R10/relax/T0.05/job"+str(i)+"/P3/"+"input"+str(j)+".txt" 
        data_name_out = "/scratch/sz8ea/FK/model/new_R10/relax/T0.05/job"+str(i)+"/P3/"+"output"+str(j)+".txt"
        #print(data_name_in)
        #print(data_name_out)       
        in_data = pd.read_csv(data_name_in,sep='\t', header=None)
        out_data = pd.read_csv(data_name_out,sep='\t', header=None)
...
df_train_matrix_y = df_train_matrix_y[:,2]
...
#%%
import joblib
joblib.dump(scaler_x, path+'X_scaler_2.pkl')
joblib.dump(scaler_y, path+'Y_scaler_2.pkl')
#%%
...
net = net.double()
if os.access(path+"model_2.pt",os.R_OK):
    net.load_state_dict(torch.load(path + "model_2.pt"))
lrspeed=1000
lr=1/lrspeed
...
#%%
torch.save(net.state_dict(),path + "model_2.pt")
#%%
